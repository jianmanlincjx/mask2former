...
def register_all_floorplan(root):
    root = os.path.join(root, "FloorPlan")
    for name, dirname in [("train", "training"), ("val", "validation")]:
        image_dir = os.path.join(root, "images", dirname)
        gt_dir = os.path.join(root, "annotations_detectron2", dirname)
        name = f"floorplan_sem_seg_{name}"
        DatasetCatalog.register(
            name, lambda x=image_dir, y=gt_dir: load_sem_seg(y, x, gt_ext="png", image_ext="png")
        )

        MetadataCatalog.get(name).set(
            stuff_classes=FLOOR_PLAN[:],
            image_root=image_dir,
            sem_seg_root=gt_dir,
            evaluator_type="sem_seg",
            ignore_label=255,
            stuff_colors= {
                            1-1: [0, 0, 0],            # background
                            2-1: [192, 192, 224],      # closet
                            3-1: [192, 255, 255],      # batchroom/washroom
                            4-1: [224, 255, 192],      # livingroom/kitchen/dining room
                            5-1: [255, 224, 128],      # bedroom
                            6-1: [255, 160, 96],       # hall
                            7-1: [255, 224, 224],      # balcony
                            8-1: [255, 60, 128],       # extra label for opening (door&window)
                            9-1: [0, 255, 0]       # extra label for wall line
                        }
...
if __name__.endswith(".builtin"):
    ...
    register_all_floorplan(_root)

# CUDA_VISIBLE_DEVICES=1 python train_net.py --config-file /data1/JM/code/Mask2Former/configs/floorplan/maskformer2_R50_bs16_160k.yaml  --num-gpus 1